---
title: Spring Cloud 实践：Nacos 服务注册发现与 Docker 启动
tags:
  - Spring Cloud
  - 微服务
  - Nacos
...
categories:
  - Spring Cloud基础系列
description: >-
  本文介绍如何在 Spring Cloud 项目中集成 Nacos，实现微服务的服务注册与发现，并通过 Docker + 数据卷启动 Nacos
  服务，保证可复现和工程化。
abbrlink: 7423fda5
date: 2026-01-11 16:47:58
---
...
在微服务体系中，我们的第一步是**让服务自动注册**，
让系统自己感知有哪些服务在运行，避免硬编码地址和频繁手动修改配置。
同时，这也是治理闭环的前提：
只有系统知道自己有哪些服务，
才能实现动态流量治理、限流、熔断和策略下发。
...
在微服务架构中，服务数量不断增加，如果没有统一的管理和发现机制，调用关系就会变得混乱。Nacos 在微服务中的角色可以理解为：
...
- **元数据存储**：为后续治理策略下发、限流规则或运维操作提供数据支持

> 🔑 今天我们先专注于 **服务注册与发现**，配置中心功能将在后续实践中展开。

---

##  项目中如何集成 Nacos

在微服务工程中，我们希望业务服务能够**自动注册到 Nacos**，实现服务发现。请配合上一篇《微服务环境配置指南》食用，Nacos 注册的核心步骤如下：
...
> 这样，网关就能动态感知业务服务的状态，保证系统请求路由和治理闭环后续功能顺利接入。



### d. 启动后观察

- 启动业务服务（如 `cloud-service-business`）
- 打开 **Nacos 控制台**（默认 `http://localhost:8848/nacos`）
- 可以看到该服务实例已经注册成功，包括 IP、端口和健康状态

> 这样，每个微服务都能被动态发现，无需手动维护调用地址，为后续流量治理和监控奠定基础。
---
