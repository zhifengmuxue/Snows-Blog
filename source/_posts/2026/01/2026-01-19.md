...
categories:
  - Spring Cloud基础系列
description: >
  通过一次真实请求的完整链路，拆解微服务中“服务发现”与“负载均衡”的职责边界， 说明 Gateway、Nacos 与 Spring Cloud
  LoadBalancer 如何协作完成实例选择。 从微服务治理的视角出发，阐述“选择”在自治系统中的意义， 以及负载均衡如何成为系统迈向自我决策的第一步。
abbrlink: 601b76c
...
在微服务体系中，请求路由并不是一个“一步到位”的过程，而是被清晰地拆分成了两个阶段：

### 服务发现：有哪些实例？

服务发现由 **Nacos** 负责。

当 `cloud-service-business` 启动多个实例后，Nacos 中维护的状态大致是这样的：
...
在实际工程中，无论是 Gateway 还是 OpenFeign，都不会自己决定调用哪一个实例，而是：
...
```text
http://localhost:9001/cloud-service-business/business/hello
```

系统内部实际发生了下面这些事情：

1. 请求进入 `cloud-gateway`
...
{% asset_img "Phase_1_Day_6_1.png" "Phase_1_Day_6_1.png" %}
...
{% asset_img "Phase_1_Day_6_2.png" "Phase_1_Day_6_2.png" %}
这不是巧合，而是系统在**持续替我做选择**。
...
在治理架构的设计中，负载均衡并不是一个“基础设施细节”，而是**自治体系的起点**。
