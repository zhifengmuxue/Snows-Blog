---
title: Spring Cloud 中 OpenFeign 服务调用异常溯源：定位问题根源的实践
tags:
  - Spring Cloud
  - OpenFeign
  - LoadBalancer
...
categories:
  - Spring Cloud基础系列
description: >-
  本文记录了在微服务项目中遇到 OpenFeign 服务调用异常的排查过程，详细分析了 Bean 创建失败的根因，并演示了如何通过引入
  spring-cloud-starter-loadbalancer 解决问题的实践经验。
abbrlink: 2660c338
...
在之前的实践中，我们已经接入了 Nacos 作为服务注册中心。
...
在当前阶段，我们并没有刻意拆分大量微服务，而是先从两个核心业务服务开始：
...
我们通过 OpenFeign 将它抽象为一个接口，并把它放置到`cloud-observer`服务中
我们在这个服务中监控所有的接口的状态
在此我就不带大家在此重复之前的内容创建一个`cloud-observer`服务了，
请参看之前的博客把它注册入nacos中

```java
@FeignClient(name = "cloud-service-business")
...
为了让系统行为更贴近真实场景，
我们在`cloud-service-business`中创建一个模拟检查系统稳定性的接口
且并没有让这个检查永远成功：
...
为了观察到`cloud-service-business` 服务是否稳定
我们在`cloud-observer`中创建接口查看
```java
@RestController
...
{% asset_img "service_list.png" "服务注册中心" %}
我们访问观测服务中的接口：
...
当 `cloud-service-business` 的稳定性检查正常返回时：

{% asset_img "stability_response.png" "正常返回" %}

此时发生的事情是：

{% mermaid %}
graph TD
    A["cloud-observer"] -->|Feign 调用| B["cloud-service-business"]
...
{% asset_img "error_500.png" "请求失败" %}

或者日志中出现：

{% asset_img "error_log.png" "日志" %}
...
它是微服务系统中风险最集中的位置
在微服务治理体系中，它是自治与治理能力的起点之一
从此刻开始，微服务工程的各个模块第一次真正“协作”起来。  
而后续所有的治理、诊断与保障能力，都会围绕这一次调用逐步展开。
